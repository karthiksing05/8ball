...
# This import statement includes I need to tune my XGBRegressor
...
class MarketPredictor(object): # v2
    ...
    """
    # the constructor
    def __init__(self, ticker, dayspast=60):
    ...
    def weight_preds(self):
        ...
        if weight >= SIGNAL1:
            weighted_preds = [(pred + (pred * (weight * CONST1))) for pred in preds]
        elif weight <= SIGNAL2:
            weighted_preds = [(pred + (pred * (weight * CONST2))) for pred in preds]
        else:
...
if __name__ == "__main__":
    ...
    mp.load_data()
    mp.fit_inital()
    preds, attrs, date = mp.predict("2021-05-17")
    weighted = mp.weight_preds()
    print(preds)
    print(weighted)
